...
def aqos_policies_creation(aqos_config):
    """
    Crea una política adaptativa de calidad de servicio (AQoS) en NetApp ONTAP
    
    Las políticas adaptativas ajustan automáticamente los límites de IOPS basándose 
    en el espacio utilizado o asignado del objeto de almacenamiento.
    
    Args:
        aqos_config: Diccionario con las configuraciones de la política AQoS:
            - name: Nombre de la política
            - vserver: Nombre de la SVM
            - expected_iops: IOPS esperados por TB
            - peak_iops: IOPS pico por TB
            - expected_iops_allocation: Tipo de asignación (allocated_space/used_space)
            - peak_iops_allocation: Tipo de asignación (allocated_space/used_space)
            - absolute_min_iops: IOPS mínimos absolutos
            - block_size: Tamaño de bloque (opcional)
    ...
    try:
        ...
        required_fields = [
            'name', 'vserver', 'expected_iops', 'peak_iops',
            'expected_iops_allocation', 'peak_iops_allocation', 'absolute_min_iops'
        ]
        missing_fields = [field for field in required_fields if field not in aqos_config]
        
        if missing_fields:
            print(f"[ERROR] Missing required fields in AQoS configuration: {', '.join(missing_fields)}")
            return False
        
        # Mostrar resumen de la política a crear
        print(f"[+] Policy Name: {aqos_config['name']}")
        print(f"[+] SVM: {aqos_config['vserver']}")
        print(f"[+] Expected IOPS: {aqos_config['expected_iops']} IOPS/TB")
        print(f"[+] Peak IOPS: {aqos_config['peak_iops']} IOPS/TB")
        print(f"[+] Absolute Min IOPS: {aqos_config['absolute_min_iops']} IOPS")
        
        # POST: Crear la política AQoS usando la API REST
        qos_policy = QosPolicy()
        qos_policy.name = aqos_config['name']
        qos_policy.svm = {"name": aqos_config['vserver']}
        
        # Configurar parámetros adaptativos
        qos_policy.adaptive = {
            "expected_iops": aqos_config['expected_iops'],
            "peak_iops": aqos_config['peak_iops'],
            "expected_iops_allocation": aqos_config['expected_iops_allocation'],
            "peak_iops_allocation": aqos_config['peak_iops_allocation'],
            "absolute_min_iops": aqos_config['absolute_min_iops']
        }
        
        # Agregar block_size si está especificado
        if 'block_size' in aqos_config and aqos_config['block_size']:
            qos_policy.adaptive['block_size'] = aqos_config['block_size']
        
        # Enviar la petición POST a la cabina
        print(f"[*] Sending creation request to NetApp cluster...")
        qos_policy.post(hydrate=True, return_timeout=0)
        
        print(f"[+] Adaptive QoS Policy created successfully!")
        print(f"[+] Policy UUID: {qos_policy.uuid}")
        
        # Verificar creación obteniendo los detalles de la política
        print(f"[*] Verifying policy creation...")
        created_policy = QosPolicy(uuid=qos_policy.uuid)
        created_policy.get()
        
        # Preparar datos para el log
        policy_data = {
            "uuid": created_policy.uuid,
            "name": created_policy.name,
            "svm": created_policy.svm.name if hasattr(created_policy.svm, 'name') else str(created_policy.svm),
            "adaptive": {
                "expected_iops": aqos_config['expected_iops'],
                "peak_iops": aqos_config['peak_iops'],
                "expected_iops_allocation": aqos_config['expected_iops_allocation'],
        ...
        # Guardar en log
        save_to_log("aqos_policy_created", policy_data)
    ...
    # CONTROL DE ERRORES
        ...
        if error.status_code == 409:
            print(f"[ERROR] Policy already exists with this name")
        elif error.status_code == 400:
            print(f"[ERROR] Invalid parameters - check policy configuration")
...
# AQOS POLICIES STEPS
print("\n" + "="*70)
print("  ADAPTIVE QOS POLICY CREATION")
print("="*70)

# Verificar que existe la configuración de aqos_policy
if 'aqos_policy' not in config_data:
    print("\n[ERROR] Missing 'aqos_policy' section in config.yaml")
    print("[ERROR] Cannot create AQoS policy without configuration")
    exit(1)

# Crear la política adaptativa de QoS
if aqos_policies_creation(config_data['aqos_policy']):
    print("\n[SUCCESS] Adaptive QoS Policy creation workflow completed!")
else:
